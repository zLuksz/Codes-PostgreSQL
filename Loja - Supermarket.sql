...
-- Crie uma função acionada por uma trigger que alimente a tabela de adutitoria de produtos.
...
-- Crie uma função chamada (registrarvenda), que, ao lançar um registro novo na tabela:
-- 		1° lance na tabela produtos_vendas todos os produtos que foram adquiridos nesta mesma venda.
--		2° atualize o estoque de cada produto, diminuindo a quantidade adquirida da quantidade em estoque.
...
DELETE FROM produtos_vendas WHERE fk_venda = 'XPTO'